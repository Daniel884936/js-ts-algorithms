 ...
 */
   ...
   const getLessThanThreeDigits = (numbers:number[]):number[] =>{
        if(numbers ==null || numbers.length <=0 ) return [];
        let numbersWithLessThanThreeDigits:number[] = numbers.filter(x=>{
            if(x<=999 && x>=-999) return true;
             return false;
            });

            let  numPositions:number[] = [];
    
            numbersWithLessThanThreeDigits.forEach((currentItem,i) => {
