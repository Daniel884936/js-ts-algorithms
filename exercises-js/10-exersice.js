...
(function () {
    ...
    var moreThanThreeDigits = function (numbers) {
        if (numbers == null || numbers.length <= 0)
            return [];
        var numbersWithMoreThanThreeDigits = numbers.filter(function (x) {
            if (x >= 1000 || x <= -1000)
                return true;
            return false;
        });
        var numPositions = [];
        numbersWithMoreThanThreeDigits.forEach(function (currentItem, i) {
